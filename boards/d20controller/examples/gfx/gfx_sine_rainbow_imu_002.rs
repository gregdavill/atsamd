...
pub fn draw(buffer: &mut [u8], imu_x: i16, imu_y: i16, imu_z: i16){
    ...
    for y in 0..panel_y as usize {
        for x in 0..panel_x as usize {
            ...
            let shift1 : f32 = (c as f32 + x as f32*(c as f32/150.0).sin()*2.0+2.1 + y as f32*(c as f32/200.0).cos()*1.0+1.1) / 40.0;

            let color: Rgb = hsv::hsv2rgb(Hsv{h: (shift1.sin() * 126.0 + 129.0) as u8, s: 255, v: ((shift * 4.0).sin()*64.0) as u8 + 65});
