...
///
/// `u32 + (u8 * 8)` = 12 bytes
const CHANNEL_REGISTERS_SIZE: u32 = 0x10;
...
impl ControllerCritical {
    ...
    pub fn configure_default_behaviors(
        ...
        peripheral_trigger: dmac::chctrla::TRIGSRC_A,
        trigger_action: dmac::chctrla::TRIGACT_A,
    ) {
        self.dmac
            .chprilvl(channel_id)
            .write(|reg| reg.prilvl().lvl0());

        self.dmac.chctrla(channel_id).modify(|_,reg| {
            reg.trigsrc()
    ...
    pub fn set_interrupt_mask(&self, channel_id: channel::Id, interrupt_mask: u8) {
        unsafe {
            ...
            self.dmac
                .chctrla(channel_id)
                .modify(|_,reg| reg.enable().set_bit());
        }
    }

    /// Set action
    pub fn set_action(&self, channel_id: channel::Id, action: dmac::chctrla::TRIGACT_A) {
        self.dmac
            .chctrla(channel_id)
            .modify(|_,reg| reg.trigact().variant(action));
    }

    /// Set trigger
    pub fn set_trigger(&self, channel_id: channel::Id, trigger: dmac::chctrla::TRIGSRC_A) {
        self.dmac
            .chctrla(channel_id)
            .modify(|_,reg| reg.trigsrc().variant(trigger));
    }
...
macro_rules! impl_channel_register {
        ($name:ident, $type:ty, $base:ident) => {
            fn $name(&self, channel_id: channel::Id) -> &$type {
                ...
                unsafe { mem::transmute(base_addr + (channel_id as u32) *CHANNEL_REGISTERS_SIZE) }
                //unsafe { mem::transmute(base_addr) }
            }
        }
    }
